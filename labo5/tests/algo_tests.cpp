...
TEST(algo_tests, test_maximo) {
    const set<int> s = {2, 3, 4};
    EXPECT_EQ(maximo(s), 4);

    const vector<int> v = {100, 2, 3, 2, 4};
    EXPECT_EQ(maximo(v), 100);

    list<int> l = {5, 4};
    EXPECT_EQ(maximo(l), 5);

    map<int, string> m;
    m[5] = "Hola";
    m[6] = "Mundo";
    EXPECT_EQ(maximo(m), (pair<const int,string>(6,"Mundo")));
}
...
TEST(algo_tests, test_ordenado) {
    ...
    EXPECT_TRUE(ordenado(v1,true));
    EXPECT_FALSE(ordenado(v1,false));
    EXPECT_FALSE(ordenado(v2,true));
    EXPECT_TRUE(ordenado(v2,false));
    EXPECT_TRUE(ordenado(s1,true));
    EXPECT_TRUE(ordenado(s2,true));
}
...
TEST(algo_tests, test_maximoIter) {
    const set<int> s = {2, 3, 4};
    EXPECT_EQ(maximoIter(s.begin(),s.end()), 4);

    const vector<int> v = {100, 2, 3, 2, 4,500};
    EXPECT_EQ(maximoIter(v.begin(),v.end()), 500);
    EXPECT_EQ(maximoIter(v.begin(),v.begin()+3), 100);

    const list<int> l = {5, 4};
    EXPECT_EQ(maximoIter(l.begin(),l.end()), 5);


    map<int, string> m;
    m[5] = "Hola";
    m[6] = "Mundo";
    EXPECT_EQ(maximoIter(m.begin(),m.end()), (pair<const int,string>(6,"Mundo")));
}
...
TEST(algo_tests, test_split) {
    ...
    set <int> s1 = {1,2,3,6,7};
    set <int> s2 = {8,9,16,45};
    ...
    EXPECT_EQ(get<0>(split(v,100)),v5);
    EXPECT_EQ(get<1>(split(v,100)),v6);
