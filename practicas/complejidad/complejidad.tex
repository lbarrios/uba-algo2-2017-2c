...
\section*{Ejercicio 3}
\framebox{\begin{minipage}{0.98\columnwidth}
	a) ¿Qué significa, intuitivamente, $O(f) \in O(g)$? ¿Qué se puede concluir acerca del crecimiento de $f$ y $g$ cuando, simultáneamente, tenemos $O(f) \in O(g)$ y $O(g) \in O(f)$?.\\
